export const COMPONENT_IDS = {
  LIST_STUFF_EDIT: 'list-stuff-edit',
  NAVBAR_ADD_STUFF: 'navbar-add-stuff',
  NAVBAR_LIST_STUFF: 'navbar-list-stuff',
  NAVBAR_USER_PROFILE: 'navbar-user-profile',
  NAVBAR_LIST_STUFF_ADMIN: 'navbar-list-stuff-admin',
  NAVBAR_CREATE_TESTIMONY: 'navbar-create-testimony',
  NAVBAR_HEARING_LIST: 'navbar-hearing-list',
  NAVBAR_TESTIMONY_LIST: 'navbar-testimony-list',
  NAVBAR_BILL_LIST: 'navbar-bill-list',
  NAVBAR_CURRENT_USER: 'navbar-current-user',
  NAVBAR_LANDING_PAGE: 'navbar-landing-page',
  NAVBAR_HOME_PAGE: 'navbar-home-page',
  NAVBAR_SIDEBAR: 'navbar-sidebar',
  NAVBAR_LOGIN: 'navbar-login',
  NAVBAR_LOGIN_DROPDOWN_SIGN_UP: 'navbar-login-dropdown-sign-up',
  NAVBAR_MANAGE_DROPDOWN: 'navbar-manage-dropdown',
  NAVBAR_MANAGE_DROPDOWN_DATABASE: 'navbar-manage-dropdown-database',
  NAVBAR_SIGN_OUT: 'navbar-sign-out',
  SIGN_IN_FORM_EMAIL: 'sign-in-form-email',
  SIGN_IN_FORM_PASSWORD: 'sign-in-form-password',
  SIGN_IN_FORM_SUBMIT: 'sign-in-form-submit',
  SIGN_UP_FORM_FIRST_NAME: 'sign-up-form-first-name',
  SIGN_UP_FORM_LAST_NAME: 'sign-up-form-last-name',
  SIGN_UP_FORM_EMAIL: 'sign-up-form-email',
  SIGN_UP_FORM_IMG: 'sign-up-form-img',
  SIGN_UP_FORM_PHONE: 'sign-up-form-phone',
  SIGN_UP_FORM_PASSWORD: 'sign-up-form-password',
  SIGN_UP_FORM_ROLE: 'sign-up-form-role',
  SIGN_UP_FORM_TESTIMONY_ROLE: 'sign-up-form-testimony-role',
  SIGN_UP_FORM_OFFICE: 'sign-up-form-office',
  SIGN_UP_FORM_TASK: 'sign-up-form-task',
  SIGN_UP_FORM_SUBMIT: 'sign-up-form-submit',
  INDIVIDUAL_BILL_BUTTON: 'individual-bill-button',
  CREATE_TESTIMONY_CHOOSE_FROM_BILLS_BUTTON: 'create-testimony-choose-from-bills-button',
  CREATE_TESTIMONY_SELECT_BILL_BUTTON: 'create-testimony-select-bill-button',
  CREATE_TESTIMONY_BILL: 'create-testimony-bill',
  CREATE_TESTIMONY_FORM_FIRST_NAME: 'create-testimony-form-first-name',
  CREATE_TESTIMONY_FORM_LAST_NAME: 'create-testimony-form-last-name',
  CREATE_TESTIMONY_FORM_POSITION: 'create-testimony-form-position',
  CREATE_TESTIMONY_FORM_TESTIFYING: 'create-testimony-form-testifying',
  CREATE_TESTIMONY_FORM_TESTIFYING_METHOD: 'create-testimony-form-testifying-method',
  CREATE_TESTIMONY_FORM_SUBMIT: 'create-testimony-form-submit',
  CREATE_ACCOUNT: 'create-account',
  SEARCH_BAR: 'search-bar',
  SEARCH_BAR_INPUT: 'search-bar-input',
};
